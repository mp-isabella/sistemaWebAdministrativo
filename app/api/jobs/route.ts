...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasRolePermission(userRole, 'create', 'jobs')) {
      return NextResponse.json({ error: "Sin permisos para crear trabajos" }, { status: 403 })
    }
    ...
    const jobData = {
      ...
      companyId,
      technicianId: technicianId || null, // Asegurar que sea null si no hay técnico
    ...
    // Debug: Mostrar datos que se van a crear
    // Verificar que las relaciones existan antes de crear el job
    await prisma.client.findUnique({ where: { id: clientId } })
    await prisma.service.findUnique({ where: { id: finalServiceId } })
    await prisma.company.findUnique({ where: { id: companyId } })
    if (technicianId) {
      await prisma.user.findUnique({ where: { id: technicianId } })
    }
    ...
    console.error('❌ API Jobs - Error al crear job:', error)
    // Manejar errores específicos de Prisma
    if (error instanceof Error) {
      if (error.message.includes('Unique constraint')) {
        return NextResponse.json({
          error: "Ya existe un trabajo con los mismos datos",
          details: error.message
        }, { status: 409 })
      }
      if (error.message.includes('Foreign key constraint')) {
        return NextResponse.json({
          error: "Error de referencia: Verifique que el cliente, servicio, empresa o técnico existan",
          details: error.message
        }, { status: 400 })
      }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (isChangingTechnician && !hasRolePermission(userRole, 'update', 'jobs')) {
      return NextResponse.json({ error: "Solo administradores y secretarias pueden cambiar técnicos" }, { status: 403 })
    }
    // Verificar permisos básicos para actualizar trabajos
    if (!hasRolePermission(userRole, 'update', 'jobs')) {
      return NextResponse.json({ error: "Sin permisos para actualizar trabajos" }, { status: 403 })
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Verificar permisos para eliminar trabajos
    const userRole = (session.user as any).role;
    if (!hasRolePermission(userRole, 'delete', 'jobs')) {
