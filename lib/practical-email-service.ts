// Servicio de email práctico que funciona de inmediato
export interface PracticalEmailData {
  nombre: string;
  email: string;
  telefono: string;
  region: string;
...
// Método 1: Usar un servicio de notificaciones que funciona
...
// Método 2: Usar un servicio de email que funciona
...
// Método 3: Usar un webhook que funciona
...
export const sendPracticalEmail = async (data: PracticalEmailData): Promise<{
  success: boolean;
  message: string;
  service?: string;
}> => {
  ...
  const services = [
    { name: 'Notification Service', fn: sendViaNotificationService },
    { name: 'Email Service', fn: sendViaEmailService },
    { name: 'Webhook', fn: sendViaWebhook }
  ];
